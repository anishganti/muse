...
class EncoderBlock(nn.Module):
    ...
    def __init__(self, hidden_size:int, num_head:int, dropout:float) -> None:
        ...
        self.self_attn = MultiHeadAttention(hidden_size, num_head, dropout)
...
class MultiAxisTransformerBlock(nn.Module):
    ...
    def __init__(self, nchannels:int, num_head:int, dropout:float) -> None:
        ...
        self.cross_attn = MultiHeadAttention(nchannels, num_head, dropout)
...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, hidden_size:int, num_head:int, dropout:float) -> None:
        ...
        self.cross_attn = MultiHeadAttention(hidden_size, num_head, dropout)
        self.mlp = MLP(hidden_size)
...
class MLP(nn.Module):
    ...
    def __init__(self, hidden_size:int) -> None:
        ...
        self.ln1 = LayerNorm(hidden_size)
        self.proj1 = nn.Linear(hidden_size, 4 * hidden_size, bias=False)
        self.act = nn.GELU(approximate='tanh')
        self.ln2 = LayerNorm(4 * hidden_size)
        self.proj2 = nn.Linear(4 * hidden_size, hidden_size, bias=False)
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, hidden_size:int, num_head:int, dropout:float) -> None:
        ...
        self.to_kv = nn.Linear(hidden_size, 2 * hidden_size, bias=False)
    ...
    def forward(self, input, context=None):
        ...
        if self.flash:
            ...
            sim = torch.einsum("b ... n d, b h t d -> b ... n t", q, k) * self.scale
            att = self.attn_dropout(sim.softmax(dim=-1))
            y = torch.einsum("b ... n t, b h t d -> b ... n d", att, v)

        y = rearrange(y, "b h ... d -> b ... (h d)")

        return self.resid_dropout(self.to_o(y))
...
class MultiAxisAttention(nn.Module):
    ...
    def forward(self, img_blocks):
        ...
        # slower operation using einsum
            ...
            sim = torch.einsum("b h n p c, b h n q c -> b h n p q", q, k) * self.scale
            att = self.attn_dropout(sim.softmax(dim=-1))
